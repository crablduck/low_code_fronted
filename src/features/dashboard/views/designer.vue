...
import type { MenuSelectOption, MenuItem } from '@/shared/types/menu'
...
import type { ChartFieldMapping } from '@/shared/utils/chartDataTransform'
import { useDatasetStore } from '@/stores/dataset'
import type { DashboardForm, ChartConfig } from '@/shared/types/dashboard'
...
const handleSave = async () => {
  try {
    ...
    // 使用持久化工具构建仪表盘数据
    const { serializeLayout, getSaveSummary } = await import('@/shared/utils/dashboardPersistence')
...
const loadDashboard = async (id: string) => {
  try {
    ...
    if (dashboardDetail.layout) {
      const { deserializeLayout } = await import('@/shared/utils/dashboardPersistence')
