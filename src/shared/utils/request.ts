...
import type { ApiResponse } from '@/shared/types/dataManagement'
import { addGlobalHeaders, logRequestInfo } from './requestHelpers'

// 创建两个axios实例
// 1. 主服务API实例（用于登录、菜单等）
const mainService: AxiosInstance = axios.create({
  baseURL: import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:6001',
  timeout: 15000,
  headers: {
...
const dataSourceService: AxiosInstance = axios.create({
  baseURL: import.meta.env.VITE_DATASOURCE_BASE_URL || 'http://127.0.0.1:8080',
  timeout: 15000,
  headers: {
    'Content-Type': 'application/json'
  }
